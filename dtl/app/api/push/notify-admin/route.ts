...
import { getAdminTokens } from "@/lib/push-tokens-store";
...
export async function POST(request: Request) {
  try {
    ...
    const details = name ? (isBooking && (date || time) ? `${name} · ${[date, time].filter(Boolean).join(" ")}` : name) : "";
    const message = {
      notification: { title, body: details || (isBooking ? "O client a trimis o cerere de programare." : "O client a trimis o cerere de ofertă.") },
      data: { type: type ?? "", name: name ?? "", date: date ?? "", time: time ?? "" },
      tokens,
    };
