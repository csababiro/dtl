...
import {
  getTestimonialByIdServer,
  updateTestimonialServer,
  removeTestimonialServer,
...
export async function PATCH(
  ...
  const item = getTestimonialByIdServer(id);
  ...
  try {
    ...
    const updates: Partial<{ author: string; role: string; text: string; rating: number; visible: boolean }> = {};
    ...
    const updated = updateTestimonialServer(id, updates);
...
export async function DELETE(
  ...
  const { id } = await params;
  const ok = removeTestimonialServer(id);
